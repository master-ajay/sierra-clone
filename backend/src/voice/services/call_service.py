...
def open_call(conn: sqlite3.Connection, line_id: str, line_key: str, settings: Settings) -> CallResponse:
    ...
    now = _now()
    conn.execute(
        "INSERT INTO voice_calls (call_id, line_id, session_id, created_at) VALUES (?, ?, ?, ?)",
        (call_id, line_id, session_id, now),
    )
    conn.commit()
    return _row_to_call(conn.execute("SELECT * FROM voice_calls WHERE call_id=?", (call_id,)).fetchone())


def exchange_turn(conn: sqlite3.Connection, call_id: str, text: str, line_key: str, settings: Settings) -> TurnResponse:
    call_row = conn.execute("SELECT * FROM voice_calls WHERE call_id=?", (call_id,)).fetchone()
    if not call_row:
        raise HTTPException(status_code=404, detail=_err("not_found", "Call not found"))

    line_row = conn.execute("SELECT * FROM voice_lines WHERE line_id=?", (call_row["line_id"],)).fetchone()
    if not line_row:
        raise HTTPException(status_code=404, detail=_err("not_found", "Line not found"))
    ...
    conn.execute(
        "UPDATE voice_calls SET sentiment_trend_json=? WHERE call_id=?",
...
def end_call(conn: sqlite3.Connection, call_id: str, line_key: str, settings: Settings) -> dict:
    ...
    now = _now()
    conn.execute(
        "UPDATE voice_calls SET status='completed', ended_at=? WHERE call_id=?",
        (now, call_id),
    )
    conn.commit()
...
def escalate_call(conn: sqlite3.Connection, call_id: str, settings: Settings) -> dict:
    ...
    conn.execute("UPDATE voice_calls SET status='escalated' WHERE call_id=?", (call_id,))
    conn.commit()
