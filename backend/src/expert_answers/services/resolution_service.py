...
def create_resolution(
    ...
    conn.execute(
        "INSERT INTO expert_answers_resolutions "
        "(resolution_id, conversation_id, adp_session_id, transcript_json, resolution_note, topic, status, created_at) "
        "VALUES (?,?,?,?,?,?,?,?)",
...
def get_resolution(conn: sqlite3.Connection, resolution_id: str) -> ResolutionResponse | None:
    row = conn.execute("SELECT * FROM expert_answers_resolutions WHERE resolution_id=?", (resolution_id,)).fetchone()
    if not row:
        return None
...
def get_resolution_transcript(conn: sqlite3.Connection, resolution_id: str) -> list[dict]:
    row = conn.execute(
        "SELECT transcript_json FROM expert_answers_resolutions WHERE resolution_id=?", (resolution_id,)
    ).fetchone()
...
def set_resolution_status(conn: sqlite3.Connection, resolution_id: str, status: str) -> None:
    conn.execute("UPDATE expert_answers_resolutions SET status=? WHERE resolution_id=?", (status, resolution_id))
    conn.commit()


def get_prior_resolutions(conn: sqlite3.Connection, topic: str, limit: int = 3) -> list[dict]:
    rows = conn.execute(
        "SELECT resolution_id, transcript_json, resolution_note FROM expert_answers_resolutions "
        "WHERE topic=? AND status='drafted' ORDER BY created_at DESC LIMIT ?",
        (topic, limit),
    ).fetchall()
    return [
        {
