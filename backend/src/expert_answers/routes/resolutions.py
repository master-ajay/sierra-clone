...
from expert_answers.services.resolution_service import (
    create_resolution,
    get_resolution,
    get_resolution_transcript,
...
def retry_resolution(resolution_id: str, settings: Settings = Depends(get_settings)):
    ...
    if not resolution:
        raise HTTPException(
            status_code=404, detail={"error": {"code": "not_found", "message": "Resolution not found", "details": {}}}
        )
    if resolution.status != "draft_failed":
        return error_response(
            "invalid_state", f"Resolution status is '{resolution.status}'; only 'draft_failed' can be retried", 400
        )
