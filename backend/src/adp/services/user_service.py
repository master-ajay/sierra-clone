...
def create_user(conn: sqlite3.Connection, data: UserCreate) -> UserResponse:
    ...
    now = _now()
    conn.execute(
        "INSERT INTO adp_users (user_id, external_id, display_name, metadata, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?)",
...
def get_user(conn: sqlite3.Connection, user_id: str) -> UserResponse | None:
    row = conn.execute("SELECT * FROM adp_users WHERE user_id = ?", (user_id,)).fetchone()
    return _row_to_user(row) if row else None


def get_user_by_external_id(conn: sqlite3.Connection, external_id: str) -> UserResponse | None:
    row = conn.execute("SELECT * FROM adp_users WHERE external_id = ?", (external_id,)).fetchone()
    return _row_to_user(row) if row else None
...
def list_users(conn: sqlite3.Connection, cursor: str | None, limit: int) -> tuple[list[UserResponse], str | None]:
    if cursor:
        rows = conn.execute(
            "SELECT * FROM adp_users WHERE created_at < ? ORDER BY created_at DESC LIMIT ?",
            (cursor, limit + 1),
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT * FROM adp_users ORDER BY created_at DESC LIMIT ?", (limit + 1,)
        ).fetchall()
...
def update_user(conn: sqlite3.Connection, user_id: str, data: UserUpdate) -> UserResponse | None:
    ...
    now = _now()
    conn.execute(
        "UPDATE adp_users SET display_name = ?, metadata = ?, updated_at = ? WHERE user_id = ?",
...
def delete_user(conn: sqlite3.Connection, user_id: str) -> bool:
    result = conn.execute("DELETE FROM adp_users WHERE user_id = ?", (user_id,))
    conn.commit()
