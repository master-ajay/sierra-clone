...
def append_message(conn: sqlite3.Connection, session_id: str, data: MessageCreate) -> MessageResponse:
    ...
    now = _now()
    conn.execute(
        "INSERT INTO adp_messages (message_id, session_id, role, content, metadata, created_at) VALUES (?,?,?,?,?,?)",
        (message_id, session_id, data.role, data.content, json.dumps(data.metadata), now),
    )
    conn.execute("UPDATE adp_sessions SET updated_at=? WHERE session_id=?", (now, session_id))
    conn.commit()
    row = conn.execute("SELECT * FROM adp_messages WHERE message_id=?", (message_id,)).fetchone()
...
def list_messages(
    ...
    if cursor:
        rows = conn.execute(
            "SELECT * FROM adp_messages WHERE session_id=? AND created_at>? ORDER BY created_at ASC LIMIT ?",
            (session_id, cursor, limit + 1),
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT * FROM adp_messages WHERE session_id=? ORDER BY created_at ASC LIMIT ?",
            (session_id, limit + 1),
        ).fetchall()
