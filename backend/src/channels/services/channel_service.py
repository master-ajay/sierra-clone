...
def create_channel(conn: sqlite3.Connection, data: ChannelCreate, settings: Settings) -> ChannelResponse:
    ...
    now = _now()
    conn.execute(
        "INSERT INTO channels_channels (channel_id, agent_id, adp_user_id, name, type, channel_key, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (channel_id, data.agent_id, adp_user_id, data.name, data.type, channel_key, now, now),
    )
    conn.execute("INSERT INTO channels_channel_stats (channel_id) VALUES (?)", (channel_id,))
    conn.commit()
...
def get_channel(conn: sqlite3.Connection, channel_id: str) -> ChannelResponse | None:
    row = conn.execute("SELECT * FROM channels_channels WHERE channel_id=?", (channel_id,)).fetchone()
...
def get_channel_by_key(conn: sqlite3.Connection, channel_key: str) -> ChannelResponse | None:
    row = conn.execute("SELECT * FROM channels_channels WHERE channel_key=?", (channel_key,)).fetchone()
    return _row_to_channel(row) if row else None
...
def list_channels(
    ...
    rows = conn.execute(f"SELECT * FROM channels_channels {where} ORDER BY created_at DESC LIMIT ?", params).fetchall()
...
def update_channel(conn: sqlite3.Connection, channel_id: str, data: ChannelUpdate) -> ChannelResponse | None:
    ...
    now = _now()
    conn.execute(
        "UPDATE channels_channels SET name=?, status=?, updated_at=? WHERE channel_id=?",
...
def delete_channel(conn: sqlite3.Connection, channel_id: str) -> bool:
    result = conn.execute("DELETE FROM channels_channels WHERE channel_id=?", (channel_id,))
    conn.commit()
