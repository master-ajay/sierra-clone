...
def write_audit(conn: sqlite3.Connection, channel_id: str, direction: str, message_clean: str, flags: list[Flag] | list[dict], allowed: bool) -> AuditRecord:
    audit_id = str(uuid.uuid4())
    serialized = json.dumps([f.model_dump() if isinstance(f, Flag) else f for f in flags])
    conn.execute(
        "INSERT INTO audit_log (audit_id, channel_id, direction, message_clean, flags, allowed, created_at) VALUES (?,?,?,?,?,?,?)",
...
def list_audit(conn: sqlite3.Connection, cursor: str | None, limit: int) -> tuple[list[AuditRecord], str | None]:
    if cursor:
        rows = conn.execute("SELECT * FROM audit_log WHERE created_at<? ORDER BY created_at DESC LIMIT ?", (cursor, limit + 1)).fetchall()
    else:
        rows = conn.execute("SELECT * FROM audit_log ORDER BY created_at DESC LIMIT ?", (limit + 1,)).fetchall()
...
def get_stats(conn: sqlite3.Connection) -> dict:
    ...
    pii = conn.execute("SELECT COUNT(*) FROM audit_log WHERE flags LIKE '%\"type\": \"pii\"%'").fetchone()[0]
    injection = conn.execute("SELECT COUNT(*) FROM audit_log WHERE flags LIKE '%\"type\": \"prompt_injection\"%'").fetchone()[0]
    rate = conn.execute("SELECT COUNT(*) FROM audit_log WHERE flags LIKE '%\"type\": \"rate_limit\"%'").fetchone()[0]
    return {
        "total_checks": total,
