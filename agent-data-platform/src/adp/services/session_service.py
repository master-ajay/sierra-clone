...
def create_session(conn: sqlite3.Connection, user_id: str, data: SessionCreate) -> SessionResponse:
    ...
    now = _now()
    conn.execute(
        "INSERT INTO sessions (session_id, user_id, metadata, started_at, updated_at) VALUES (?, ?, ?, ?, ?)",
...
def list_sessions(
    ...
    if status and cursor:
        rows = conn.execute(
            "SELECT * FROM sessions WHERE user_id=? AND status=? AND started_at<? ORDER BY started_at DESC LIMIT ?",
            (user_id, status, cursor, limit + 1),
        ).fetchall()
    elif status:
        rows = conn.execute(
            "SELECT * FROM sessions WHERE user_id=? AND status=? ORDER BY started_at DESC LIMIT ?",
            (user_id, status, limit + 1),
        ).fetchall()
    elif cursor:
        rows = conn.execute(
            "SELECT * FROM sessions WHERE user_id=? AND started_at<? ORDER BY started_at DESC LIMIT ?",
...
def update_session(conn: sqlite3.Connection, session_id: str, data: SessionUpdate) -> SessionResponse | None:
    ...
    now = _now()
    conn.execute(
        "UPDATE sessions SET metadata=?, status=?, updated_at=? WHERE session_id=?",
