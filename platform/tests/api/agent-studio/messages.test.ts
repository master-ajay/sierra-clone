...
import { createConversation, getConversation } from '@/lib/agent-studio/conversations';

vi.mock('@/lib/agent-studio/groq', async () => {
  const actual = await vi.importActual<typeof import('@/lib/agent-studio/groq')>('@/lib/agent-studio/groq');
  return {
    ...actual,
    getGroqClient: () => ({
      chat: {
...
const { POST } = await import('@/app/api/agent-studio/conversations/[id]/messages/route');
...
describe('POST /api/agent-studio/conversations/[id]/messages', () => {
  it('streams the assistant reply and persists the conversation', async () => {
    ...
    const req = new Request('http://localhost/api/agent-studio/conversations/x/messages', {
      method: 'POST',
