...
- [x] Task 0: Scaffold — `agent-runtime/` dir tree (`src/agent_runtime/{ingestion,retrieval,generation,guardrails}`, `tests/`, `docs/`), `pyproject.toml` (setuptools src-layout, pytest config, ruff config), `requirements.txt`. **Dependencies NOT YET INSTALLED** — no `.venv` exists yet (`python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt`, run from `agent-runtime/`, was interrupted by user before completing — resume by running it, not by re-scaffolding).
- [ ] Task 1 / M1: Ingestion pipeline — `src/agent_runtime/ingestion/loader.py` (load + chunk `.txt`/`.md`/`.pdf`, ~300-500 tokens/chunk with overlap), `indexer.py` (write to ChromaDB + BM25). Test: `tests/test_ingestion.py`. Done when: ingesting a sample docs folder produces a queryable index; a test confirms chunk count and retrievability of a known passage. Needs a small sample knowledge base under `agent-runtime/docs/` (create 2-3 short `.md` fixture files as part of this task).
